...
; 1x sine movement
; Blitter clears the playfield


; Execution time 68000: 267 rasterlines
...
ss_clear_playfield1
	...
	move.l	pf1_construction1(a3),BLTDPT-DMACONR(a6)
	move.w	#pf1_plane_width-data_fetch_width,BLTDMOD-DMACONR(a6)
	move.w	#((pf1_y_size3*pf1_depth3)<<6)|(pixel_per_line/WORD_BITS),BLTSIZE-DMACONR(a6)
	rts


	CNOP 0,4
ss_get_xy_coordinates
	lea	sine_table(pc),a0
	lea	ss_xy_coordinates(pc),a1
	moveq	#0,d2			; 1st x
	move.w	ss_text_y_angle(a3),d3	; 1st y angle
	move.w	d3,d4
	subq.w	#ss_text_y_angle_speed*WORD_SIZE,d4 ; next y angle
...
ss_get_xy_coordinates_loop
	...
	move.w	d1,(a1)+		; x position
	MULUF.W pf1_plane_width*pf1_depth3,d0,d1 ; y offset in playfield
	move.w	d0,(a1)+		; y position
	addq.w	#ss_text_y_angle_step*WORD_SIZE,d3
	addq.w	#ss_text_columns_x_size,d2 ; next column in playfield
	and.w	d6,d3			; remove overflow
	dbf	d7,ss_get_xy_coordinates_loop
	rts
...
ss_sine_scroll
	movem.l a3-a5,-(a7)
	bsr.s	ss_sine_scroll_init
	move.w	#(($ff<<8)<<(8-ss_text_columns_x_size))&$ffff,d2 ; mask
	move.l	extra_pf1(a3),d3
	addq.l	#WORD_SIZE,d3		; skip 16 pixel
	move.l	pf1_construction2(a3),d4
	moveq	#DMAB_BLTDONE-8,d5
	move.w	#((ss_sine_char_y_size*ss_sine_char_depth)<<6)|(ss_sine_char_x_size/WORD_BITS),d6 ; BLTSIZE
	lea	ss_xy_coordinates(pc),a0
	lea	BLTALWM-DMACONR(a6),a1
	lea	BLTBPT-DMACONR(a6),a2
	lea	BLTAPT-DMACONR(a6),a3
	lea	BLTDPT-DMACONR(a6),a4
	lea	BLTSIZE-DMACONR(a6),a5
	MOVEF.W	ss_text_columns_number-1,d7
ss_sine_scroll_loop
	movem.w (a0)+,d0-d1		; x offset / y offset
	add.w	d0,d1			; x offset + y offset
	add.l	d4,d1			; add destination address
	add.l	d3,d0			; add source address
	WAITBLITQ d5
	move.w	d2,(a1)			; BLTALWM
	move.l	d1,(a2)			; BLTBPT playfield read
...
ss_sine_scroll_init
	...
	moveq	#pf1_plane_width-ss_sine_char_width,d0
	move.w	d0,BLTBMOD-DMACONR(a6)
	move.w	#extra_pf1_plane_width-ss_sine_char_width,BLTAMOD-DMACONR(a6)
...
ss_horiz_scrolltext
	...
	ADDF.L	ss_text_char_x_restart/8,d1
	WAITBLIT
